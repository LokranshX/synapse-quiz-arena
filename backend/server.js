...
const io = socketIo(server, {
  cors: {
    // --- ИЗМЕНЕНИЕ (CORS Origin) ---
    origin: "https://lokranshx.github.io", // Теперь разрешаем подключение с вашего GitHub Pages домена
    // --- КОНЕЦ ИЗМЕНЕНИЯ ---
    methods: ["GET", "POST"]
  }
});
    ...
    const response = await axios.post(
      'https://openrouter.ai/api/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: "system",
            // Очень строгий системный промпт: отвечать только JSON и на русском
...
io.on('connection', (socket) => {
  ...
  function sendNextQuestion(roomId) {
    ...
    if (room.currentQuestionIndex < room.questions.length) {
      ...
      // --- УДАЛЕН БЛОК ТАЙМЕРА АВТОМАТИЧЕСКОГО ПЕРЕХОДА ---
